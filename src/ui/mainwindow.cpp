...
{
    if (watched == m_ui->messageEdit && event->type() == QEvent::KeyPress) {
        auto *keyEvent = static_cast<QKeyEvent *>(event);
        const int key = keyEvent->key();
        const bool isEnterKey = key == Qt::Key_Return || key == Qt::Key_Enter;
        if (isEnterKey) {
            if (keyEvent->modifiers() & Qt::ControlModifier) {
                m_ui->messageEdit->insertPlainText(QStringLiteral("\n"));
            } else {
                sendTextMessage();
            }
            return true;
...
void MainWindow::connectUi()
{
    ...
    connect(m_ui->manualPeerButton, &QPushButton::clicked, this, &MainWindow::addManualPeer);
    connect(m_ui->hiddenPeersButton, &QPushButton::clicked, this, &MainWindow::manageHiddenPeers);
...
{
    QDialog dialog(this);
    dialog.setWindowTitle(QStringLiteral("手动添加联系人"));
    ...
    auto *hintLabel = new QLabel(QStringLiteral("当你已知对方的 IP 地址和 TCP 端口，但自动发现暂时没有找到对方时，可以使用这里手动添加。"), &dialog);
    auto *buttons = new QDialogButtonBox(QDialogButtonBox::Ok | QDialogButtonBox::Cancel, &dialog);

    nameEdit->setPlaceholderText(QStringLiteral("可选，填写联系人备注名"));
    addressEdit->setPlaceholderText(QStringLiteral("IPv4 地址，例如 192.168.1.23"));
    portSpin->setRange(1, 65535);
    portSpin->setValue(45455);
    hintLabel->setWordWrap(true);

    formLayout->addRow(QStringLiteral("名称"), nameEdit);
    formLayout->addRow(QStringLiteral("IP 地址"), addressEdit);
    formLayout->addRow(QStringLiteral("端口"), portSpin);
    ...
    const QHostAddress address(addressEdit->text().trimmed());
    if (address.isNull() || address.protocol() != QAbstractSocket::IPv4Protocol) {
        QMessageBox::warning(this, QStringLiteral("IP 地址无效"), QStringLiteral("请输入有效的 IPv4 地址。"));
        return;
    }

    const PeerInfo peer = m_manager->addManualPeer(nameEdit->text(), address, static_cast<quint16>(portSpin->value()));
    if (peer.id.isEmpty()) {
        QMessageBox::warning(this, QStringLiteral("添加失败"), QStringLiteral("联系人添加失败，请检查 IP 地址和端口。"));
        return;
    }

    m_ui->sidebarTabs->setCurrentWidget(m_ui->peersTab);
    for (int row = 0; row < m_ui->peerList->count(); ++row) {
        QListWidgetItem *item = m_ui->peerList->item(row);
        if (item && item->data(Qt::UserRole).toString() == peer.id) {
            m_ui->peerList->setCurrentItem(item);
            break;
        }
    }

    showStatusMessage(QStringLiteral("已添加手动联系人：%1").arg(peer.name), 4000);
}
...
void MainWindow::showPeerContextMenu(const QPoint &position)
{
    ...
    {
        QMenu menu(this);
        QAction *removeAction = menu.addAction(QStringLiteral("隐藏联系人"));
        QAction *manageHiddenAction = menu.addAction(QStringLiteral("隐藏联系人管理"));
        const QAction *selectedAction = menu.exec(m_ui->peerList->viewport()->mapToGlobal(position));
        if (selectedAction == removeAction) {
            removeSelectedPeer();
        } else if (selectedAction == manageHiddenAction) {
            manageHiddenPeers();
        }
        return;
    }
...
void MainWindow::removeSelectedPeer()
{
    ...
    const QString name = peerName(peerId);
    {
        const auto result = QMessageBox::question(this,
                                                  QStringLiteral("隐藏联系人"),
                                                  QStringLiteral("确认隐藏联系人“%1”？\n隐藏后可在“隐藏联系人管理”中恢复。").arg(name));
        ...
        if (m_manager->removePeer(peerId)) {
            refreshConversationView();
            showStatusMessage(QStringLiteral("已隐藏联系人：%1").arg(name), 4000);
        }
        return;
    }
...
void MainWindow::manageHiddenPeers()
{
    const QList<PeerInfo> hiddenPeers = m_manager->hiddenPeers();

    QDialog dialog(this);
    dialog.setWindowTitle(QStringLiteral("隐藏联系人管理"));

    auto *layout = new QVBoxLayout(&dialog);
    auto *hintLabel = new QLabel(hiddenPeers.isEmpty()
                                     ? QStringLiteral("当前没有被隐藏的联系人。")
                                     : QStringLiteral("选择一个隐藏联系人，可将其恢复到主联系人列表。"),
                                 &dialog);
    auto *listWidget = new QListWidget(&dialog);
    auto *buttons = new QDialogButtonBox(QDialogButtonBox::Close, &dialog);
    auto *restoreButton = buttons->addButton(QStringLiteral("恢复选中联系人"), QDialogButtonBox::AcceptRole);

    hintLabel->setWordWrap(true);
    for (const PeerInfo &peer : hiddenPeers) {
        QString label = QStringLiteral("%1  %2:%3")
                            .arg(peer.name,
                                 peer.address.toString(),
                                 QString::number(peer.port));
        if (peer.lastSeen.isValid()) {
            label.append(QStringLiteral("  [上次在线 %1]").arg(peer.lastSeen.toString(QStringLiteral("MM-dd HH:mm"))));
        }
        auto *item = new QListWidgetItem(label, listWidget);
        item->setData(Qt::UserRole, peer.id);
    }

    restoreButton->setEnabled(listWidget->count() > 0);
    ...
    connect(restoreButton, &QPushButton::clicked, &dialog, [&]() {
        ...
        if (!m_manager->restorePeer(peerId)) {
            QMessageBox::warning(&dialog, QStringLiteral("恢复失败"), QStringLiteral("隐藏联系人恢复失败。"));
            return;
        }

        showStatusMessage(QStringLiteral("联系人已恢复"), 4000);
        dialog.accept();
    });

...
QList<PeerInfo> PeerManager::peers() const
{
    ...
    for (auto it = m_peers.constBegin(); it != m_peers.constEnd(); ++it) {
        if (!m_removedPeerIds.contains(it.key())) {
...
QList<PeerInfo> PeerManager::hiddenPeers() const
{
    QList<PeerInfo> list;
    for (auto it = m_peers.constBegin(); it != m_peers.constEnd(); ++it) {
        if (m_removedPeerIds.contains(it.key())) {
...
PeerInfo PeerManager::addManualPeer(const QString &displayName, const QHostAddress &address, quint16 port)
{
    if (address.isNull() || address.protocol() != QAbstractSocket::IPv4Protocol || port == 0) {
        return {};
    }
    ...
    for (auto it = m_peers.begin(); it != m_peers.end(); ++it) {
        if (it->address == address && it->port == port) {
            if (!displayName.trimmed().isEmpty()) {
                it->name = displayName.trimmed();
            }
            m_removedPeerIds.remove(it.key());
            saveRemovedPeers();
            emitPeersChanged();
            return it.value();
        }
    ...
    PeerInfo peer;
    peer.id = QStringLiteral("manual:%1").arg(newId());
    peer.name = displayName.trimmed().isEmpty()
        ? QStringLiteral("%1:%2").arg(address.toString(), QString::number(port))
        : displayName.trimmed();
...
bool PeerManager::restorePeer(const QString &peerId)
{
    if (peerId.isEmpty() || !m_peers.contains(peerId) || !m_removedPeerIds.contains(peerId)) {
        return false;
    }

    m_removedPeerIds.remove(peerId);
    saveRemovedPeers();
    emitPeersChanged();
    return true;
}
...
void PeerManager::onPeerAnnounced(const PeerInfo &peer)
{
    ...
    const bool changed = current.id != peer.id
...
void PeerManager::emitPeersChanged()
{
    ...
    std::sort(list.begin(), list.end(), [](const PeerInfo &left, const PeerInfo &right) {
        ...
        const QString leftName = left.name.trimmed().toLower();
        const QString rightName = right.name.trimmed().toLower();
...
void PeerManager::registerConnection(PeerConnection *connection, const QString &peerId, const QString &peerName)
{
    ...
    PeerInfo peer = m_peers.value(peerId);
    const QHostAddress socketAddress = connection->socket() ? connection->socket()->peerAddress() : QHostAddress();
    const bool changed = peer.id != peerId
        || peer.name != peerName
...
void PeerManager::loadKnownPeers(QSettings &settings)
{
    const QByteArray knownPeersData = settings.value(QStringLiteral("%1/knownPeers").arg(m_settingsScope)).toByteArray();
    if (knownPeersData.isEmpty()) {
        return;
    }

    const QJsonDocument document = QJsonDocument::fromJson(knownPeersData);
    if (!document.isArray()) {
        return;
    }

    const QJsonArray peersArray = document.array();
    for (const QJsonValue &value : peersArray) {
        ...
        PeerInfo peer;
        peer.id = object.value(QStringLiteral("id")).toString().trimmed();
        if (peer.id.isEmpty() || peer.id == m_localPeerId) {
            continue;
        }

        peer.name = object.value(QStringLiteral("name")).toString().trimmed();
        peer.address = QHostAddress(object.value(QStringLiteral("address")).toString().trimmed());
...
void PeerManager::saveKnownPeers() const
{
    ...
    for (auto it = m_peers.constBegin(); it != m_peers.constEnd(); ++it) {
        ...
        object.insert(QStringLiteral("lastSeen"), peer.lastSeen.isValid() ? peer.lastSeen.toString(Qt::ISODate) : QString());
        peersArray.append(object);
    }

    QSettings settings;
    settings.setValue(QStringLiteral("%1/knownPeers").arg(settingsScope()),
                      QJsonDocument(peersArray).toJson(QJsonDocument::Compact));
}
